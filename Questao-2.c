...
double calcularForcaPonderada(double forcas[11]) {
    double G = forcas[0];
    double L1 = forcas[1];
    double L2 = forcas[4];
    double Z1 = forcas[2];
    ...
    return (8 * G + 10 * (L1 + L2) + 5 * (Z1 + Z2) + 8 * (V1 + V2) + 11 * (M1 + M2) + 12 * (A1 + A2)) / 100;
}
...
int main() {
    ...
    double forcaPonderadaTime1 = calcularForcaPonderada(forcasTime1);
    double forcaPonderadaTime2 = calcularForcaPonderada(forcasTime2);
